...
#neigh.fit(train_scaled, etiqueta.astype('int64'))
#y_pred = neigh.predict(test_scaled)

#clf = SVC(kernel='linear')
#clf.fit(train_scaled, etiqueta.astype('int64'))
#y_pred = clf.predict(test_scaled)


#tree = RandomForestClassifier(n_estimators=100, random_state=42)
#tree.fit(train_scaled, etiqueta.astype('int64'))
#y_pred = tree.predict(test_scaled)
# Generar datos de ejemplo (sustituye esto con tus datos reales)

# Crear el modelo
model = Sequential()
model.add(Dense(128, input_dim=len(train_scaled[0]), activation='relu'))
...
# Compilar el modelo con un optimizador avanzado y función de pérdida adecuada
model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])

# Entrenar el modelo
model.fit(train_scaled, etiqueta.astype('int64'), epochs=20, batch_size=64, validation_split=0.2)
...
with open("prueba16.csv", "w", newline="") as archivo:
